...
def cubes_around( cubes, coords ):
    x,y,z,w = coords
    l = [ cubes[(x_,y_,z_,w_)] for x_ in [x-1,x,x+1] for y_ in [y-1,y,y+1]
                               for z_ in [z-1,z,z+1] for w_ in [w-1,w,w+1]
          if (x_,y_,z_,w_) != coords and (x_,y_,z_,w_) in cubes ]
    return l.count( True )
